...
import PLTBGraphicHistories from "~/partials/histories/pltb/pltb.graphic.histories";
import { VIEW_MODEL } from "~/json/view-model-json";
import PLTBTableHistories from "~/partials/histories/pltb/pltb.table.histories";
import { LOG_PERIODE } from "~/json/log-periode-json";
import WrapperHistories from "~/partials/histories/wrapper.histories";

export default function PLTBHistories() {

    const group = SOCKET_TAG.pltb
            ...
            >
                {
                    model() === VIEW_MODEL.table.value
                        ? <PLTBTableHistories model={model()} periode={periode()} rows={rows()} />
                        : <PLTBGraphicHistories model={model()} periode={periode()} rows={rows()} />
                }
            </WrapperHistories>
        </ContentLayout>
    )
