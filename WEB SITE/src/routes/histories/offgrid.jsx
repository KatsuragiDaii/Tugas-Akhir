...
import OffGridTableHistories from "~/partials/histories/offgrid/offgrid.table.histories";
import OffGridGraphicHistories from "~/partials/histories/offgrid/offgrid.graphic.histories";
...
export default function OffGridHistories() {

    const group = SOCKET_TAG.pltsOffGrid

    const [firstDate, setFirstDate] = createSignal(new Date());
    const [lastDate, setLastDate] = createSignal(new Date());

    const [periode, setPeriode] = createSignal(LOG_PERIODE.daily.value); const [rows, setRows] = createSignal([]);
        ...
        <ContentLayout title={`${group.toUpperCase()} Histories`}>
                        ...
                        >
            {
                model() === VIEW_MODEL.table.value
                    ? <OffGridTableHistories model={model()} periode={periode()} rows={rows()} />
                    : <OffGridGraphicHistories model={model()} periode={periode()} rows={rows()} />
            }
</WrapperHistories>
        </ContentLayout>
    )
